...
class ModelDownloader(private val modelsDir: File) {
    ...
    companion object {
        ...
        val MODELS = listOf(
            ...
            Spec(
                label = "STT (Moonshine base)",
                url = "$SHERPA/asr-models/sherpa-onnx-moonshine-base-en-int8.tar.bz2",
                target = "stt",
                archive = true,
                approxBytes = 250_807_309,
            ),
            Spec(
                label = "TTS (Piper)",
                url = "$SHERPA/tts-models/vits-piper-en_US-lessac-medium.tar.bz2",
                target = "tts",
                archive = true,
                approxBytes = 67_230_653,
            ),
            Spec(
