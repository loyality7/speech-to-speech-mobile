 ...
 *
 * The default [ContextEngine] — everything in a bounded in-memory deque, no
 * persistence beyond explicit [toJson]/[fromJson]. A plugin can supply a
 * different [ContextEngine] (SQLite, a vector store, a remote memory service)
 * without [com.s2s.mobile.S2SEngine] changing.
 *
...
class ChatHistory(
     ...
     */
    override fun replaceLastUser(text: String) = synchronized(lock) {
     ...
     */
    override fun dropLastUserIfUnanswered() = synchronized(lock) {
    ...
    override fun messages(extraSystem: String?): List<ChatMessage> = synchronized(lock) {
        buildList {
    ...
    override fun setSystemPrompt(prompt: String) = synchronized(lock) { system = prompt }
    ...
    override fun toJson(): String = synchronized(lock) {
    ...
    override fun fromJson(json: String) = synchronized(lock) {
